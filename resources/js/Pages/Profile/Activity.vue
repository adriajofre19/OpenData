...
const props = defineProps(['plans', 'activities']);
...
function update() {
    form.get('/profile/updateactivity/'+ form.id, form);
}
...
function openEditActivityModal(id) {
    editUser.value = true;
    form.id = id;
    form.name = props.activities.name;
    form.description = props.activities.description;
    form.id_plan = props.activities.id_plan;
}
...
<template>
    ...
    <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <a href="#">
            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ activities.name }}</h5>
        </a>
        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ activities.description }}</p>
        <button type="button" @click="openEditActivityModal(activities.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
            <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.3 4.8 2.9 2.9M7 7H4a1 1 0 0 0-1 1v10c0 .6.4 1 1 1h11c.6 0 1-.4 1-1v-4.5m2.4-10a2 2 0 0 1 0 3l-6.8 6.8L8 14l.7-3.6 6.9-6.8a2 2 0 0 1 2.8 0Z"/>
            </svg>
        </button>
        <button @click="showModal(activities.id)" type="button" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
            <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
