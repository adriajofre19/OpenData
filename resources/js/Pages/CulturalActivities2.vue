...
function getAllCities() {
  axios.get('https://analisi.transparenciacatalunya.cat/resource/rhpv-yr4f.json')
    .then(response => {
      const poblaciones = response.data.map(item => item.comarca);
      const poblacionesUnicas = [...new Set(poblaciones.map(city => {
        if (city) {
...
async function getInfoAboutCity() {
  if (selectedCity.value && selectedCity.value.name) {
    ...
    try {
      ...
      const cityData = response.data.find(item => item.comarca && item.comarca.includes(cityName));

      if (cityData) {
        comarcaName = cityData.comarca;
        console.log(comarcaName)
      } else {
        console.error(`No se encontró la comarca para la ciudad "${cityName}"`);
        return;
      }
      ...
      while (true) {
        const response = await axios.get(`https://analisi.transparenciacatalunya.cat/resource/rhpv-yr4f.json?comarca=${comarcaName}&$limit=${pageSize}&$offset=${page * pageSize}`);
...
<template>
  ...
  <Head title="Activitats Culturals" />
                ...
                <div class="flex-none sm:flex">
                  ...
                  <div class="flex-auto sm:ml-5 justify-evenly">
                    ...
                    <div class="flex pt-2  text-sm text-gray-400">
                      ...
                      <a :href="route('culturalspaces.getSpaceCulture2', { id: record.codi })" target="_blank" class="flex-no-shrink bg-blue-600 px-5 ml-4 py-2 text-xs shadow-sm font-medium tracking-wider border-2 text-white rounded-md transition ease-in duration-300">Més informació</a>
                    </div>
                  </div>
                </div>
              </div>
