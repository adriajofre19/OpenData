...
function getSesionUser(){
  axios.get('/getsessionuser').then(response => {
      form.id_user = response.data.id;
  });
}

function getSpacesById(id){
  axios.get('https://analisi.transparenciacatalunya.cat/resource/rhpv-yr4f.json?codi='+id)
  .then(response => {
                            ...
                            <div class="flex-auto sm:ml-5 justify-evenly">
                                <div class="flex items-center justify-between sm:mt-2">
                                    <div class="flex items-center">
                                        <div class="flex flex-col">
                                            <div class="w-full flex-none text-lg text-gray-900 font-bold leading-none">{{ record.denominaci }}</div>
                                            <div class="flex-auto text-gray-400 my-1">
                                                <div v-if="record.adre_a !== undefined || record.espai !== undefined || record.entrades !== undefined">
                                                    <span v-if="record.adre_a !== undefined" class="mr-3">{{ record.adre_a }}</span>
                                                    <span v-else class="mr-3">No disponible</span>
                                                    <span class="mr-3 border-r border-gray-600 max-h-0"></span>
                                                    <span v-if="record.espai !== undefined" class="mr-3">Espai: {{ record.espai }}</span>
                                                    <span v-else class="mr-3">Espai: No disponible</span>
                                                    <span class="mr-3 border-r border-gray-600 max-h-0"></span>
                                                    <span v-if="record.entrades !== undefined" class="mr-3">Preu: {{ record.entrades }}</span>
                                                    <span v-else class="mr-3">Preu: No disponible</span>
                                                    <span class="mr-3 border-r border-gray-600 max-h-0"></span>
                                                    <span v-if="record.horari !== undefined" class="mr-3">Horari: {{ record.horari }}</span>
                                                    <span v-else class="mr-3">Horari: No disponible</span>
                                                </div>                        
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="flex pt-2  text-sm text-gray-400">
                                    ...
                                    <form @submit.prevent="addFavourite">                
                                        <a target="_blank" class="flex-no-shrink bg-blue-600 px-5 ml-4 py-2 text-xs shadow-sm hover:shadow-lg font-medium tracking-wider text-white rounded-md transition ease-in duration-300">AFEGIR A PREFERITS</a>
                                    </form>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex-none sm:flex">
                            ...
                            <div class="flex-auto sm:ml-5 justify-evenly">
                                <div class="flex items-center justify-between sm:mt-2">
                                    <div class="flex items-center">
                                        <div class="flex flex-col">
                                            <div class="w-full flex-none text-lg text-gray-900 font-bold leading-none">Descripcio:</div>
                                            <div class="flex-auto text-gray-400 my-1">
                                                <span v-if="record.descripcio" class="mr-3">{{ record.descripcio }}</span>
                                                <span v-else class="mr-3">No disponible</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center mr-auto">
                                ...
                                <div class="flex flex-col ml-3">
                                    ...
                                    <p v-if="record.tel_fon" class="text-sm text-gray-500 leading-none mt-1">{{ record.tel_fon }}</p>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center mr-auto">
                                ...
                                <div class="flex flex-col ml-3 min-w-0">
                                    ...
                                    <p v-if="record.adre_a" class="text-sm text-gray-500 leading-none mt-1 truncate">{{ record.adre_a }}</p>
                                    <p v-else class="text-sm text-gray-500 leading-none mt-1 truncate">No disponible</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center mr-auto">
                                ...
                                <div class="flex flex-col ml-3 min-w-0">
                                    ...
                                    <p v-if="record.enlla_os" class="text-sm text-gray-500 leading-none mt-1 truncate">{{ record.enlla_os.split(',')[0].trim() }}</p>
                                    <p v-else class="text-sm text-gray-500 leading-none mt-1 truncate">No disponible</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <Carousel :value="products" :numVisible="1" :numScroll="1" orientation="vertical" verticalViewPortHeight="330px" contentClass="flex align-items-center">
                        <template #item="slotProps">
                            <div class="border-1 surface-border border-round m-2  p-3">
                                <figure class="flex flex-col items-center justify-center p-8 text-center bg-white rounded-md  dark:bg-gray-800 shadow-md" style="margin: 10px;">
