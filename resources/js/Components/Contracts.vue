...
if (props.contracts.description) {
    const { duration: contractDuration, features: contractFeatures } = JSON.parse(props.contracts.description);
    duration.value = contractDuration;
    features.value = contractFeatures;
}

const update = () => {
    form.post('/adminpanel/updatecontract/' + form.id, form);
};

function deleteContract(id) {
    form.delete('/adminpanel/deletecontract/' + id);
}
...
function openEditContractModal(id) {
    editUser.value = true;
    form.id = id;
    form.name = props.contracts.name;
    form.type = props.contracts.type;
    form.contract_description = props.contracts.contract_description;
    form.price = props.contracts.price;
}
...
<template>
    ...
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        <td class="px-6 py-4">{{ contracts.name }}</td>
        <td class="px-6 py-4">{{ contracts.contract_description }}</td>
        <td class="px-6 py-4">{{ duration }}</td>
        <td class="px-6 py-4">
            <template v-if="features.length > 0">
                <select class="block w-full text-gray-700 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <option v-for="(feature, index) in features" :key="index">{{ index + 1 }}. {{ feature }}</option>
                </select>
            </template>
            <template v-else>
                <div>No hi ha característiques disponibles.</div>
            </template>
        </td>
        <td class="px-6 py-4">{{ contracts.price }}</td>
        <td class="px-6 py-4">
            <button type="button" @click="openEditContractModal(contracts.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.3 4.8 2.9 2.9M7 7H4a1 1 0 0 0-1 1v10c0 .6.4 1 1 1h11c.6 0 1-.4 1-1v-4.5m2.4-10a2 2 0 0 1 0 3l-6.8 6.8L8 14l.7-3.6 6.9-6.8a2 2 0 0 1 2.8 0Z"/>
                </svg>
            </button>
            <button @click="showModal(contracts.id)" type="button" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <form class="p-4 md:p-5" @submit.prevent="update">
                    ...
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            ...
                            <input v-model="form.price" type="number" name="price" id="price" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"  required="">
                        </div>
                    </div>
                    <button type="submit" data-modal-target="adduser-modal" data-modal-toggle="adduser-modal" class="text-white inline-flex items-center bg-gray-900  focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
