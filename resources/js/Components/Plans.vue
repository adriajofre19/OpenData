...
const props = defineProps(['plans', 'categories']);
...
function update() {
    form.get('/adminpanel/updateplan/'+ form.id, form);
}

function deletePlan(id) {
    form.delete('/adminpanel/deleteplan/' + id);
}
...
<template>
    ...
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        <td class="px-6 py-4">{{ plans.title }}</td>
        <td class="px-6 py-4">{{ plans.description }}</td>
        <td class="px-6 py-4">{{ plans.start_date }}</td>
        <td class="px-6 py-4">{{ plans.end_date }}</td>
        <td class="px-6 py-4">{{ plans.category_name }}</td>
        <td class="px-6 py-4">{{ plans.user_name }}</td>
        <td class="px-6 py-4">
            <button type="button" @click="openEditPlanModal(plans.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.3 4.8 2.9 2.9M7 7H4a1 1 0 0 0-1 1v10c0 .6.4 1 1 1h11c.6 0 1-.4 1-1v-4.5m2.4-10a2 2 0 0 1 0 3l-6.8 6.8L8 14l.7-3.6 6.9-6.8a2 2 0 0 1 2.8 0Z"/>
                </svg>
            </button>
            <button @click="showModal(plans.id)" type="button" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
